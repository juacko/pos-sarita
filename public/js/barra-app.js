      ...
      if (!pedidos || pedidos.length === 0) {
        container.innerHTML = `
          <div class="empty-state" style="grid-column:1/-1;">
            ...
            <p>Los pedidos nuevos de barra aparecerán aquí automáticamente</p>
          </div>`;
                ...
                <div class="comanda-item ${item.estado === 'LISTO' ? 'listo' : ''}">
                  ...
                  <div style="display:flex;align-items:center;flex:1;">
                    ...
                    <div class="comanda-item-info">
                      ...
                      ${item.variante_nombre ? `<div style="font-size:0.75rem;color:#4F46E5;font-weight:600;">${item.variante_nombre}</div>` : ''}
                  ...
                  <span class="comanda-item-status badge-${item.estado === 'PENDIENTE' ? 'yellow' : item.estado === 'COCINANDO' ? 'blue' : 'green'} badge">
                    ${item.estado === 'COCINANDO' ? 'PREPARANDO' : item.estado}
                  </span>
                </div>`;
              }).join('');
    ...
    function marcarPreparando(pedidoId) {
      fetch(`/api/pedidos/${pedidoId}/items/estado`, {
        ...
        body: JSON.stringify({ estado: 'COCINANDO', destino: 'barra' })
    ...
    function cargarHistorial() {
      ...
      fetch('/api/pedidos/barra/historial' + qs)
        .then(r => r.json())
                ...
                ${(p.items || []).map(i => `
                  <div class="comanda-item">
                    ...
                    <div class="comanda-item-info">
                      ...
                      ${i.variante_nombre ? `<div style="font-size:0.75rem;color:#4F46E5;font-weight:600;">${i.variante_nombre}</div>` : ''}
    ...
    async function openStockModal() {
      document.getElementById('stockBarraModal').classList.add('active');
      try {
        const res = await fetch('/api/productos');
        if (res.ok) {
          productosBarra = await res.json();
          renderStockBarraList();
        }
      } catch (e) { console.error(e); }
    }

    function closeStockModal() {
      document.getElementById('stockBarraModal').classList.remove('active');
    }

    function renderStockBarraList() {
      const container = document.getElementById('stockBarraList');
      if (!container) return;
      const q = (document.getElementById('stockBarraSearch')?.value || '').trim().toLowerCase();
      let list = productosBarra.filter(p => p.nombre.toLowerCase().includes(q) || (p.categoria || '').toLowerCase().includes(q));

      if (!list.length) {
        container.innerHTML = '<p style="text-align:center; color:#64748B; padding:20px;">No se encontraron productos</p>';
        return;
      }
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; gap:8px; flex-wrap:wrap;">
            ...
            <div style="display:flex; align-items:center; gap:8px;">
              <label style="display:flex; align-items:center; gap:4px; font-size:0.8rem; font-weight:600; cursor:pointer;">
                <input type="checkbox" ${activo ? 'checked' : ''} onchange="toggleStockBarra(${p.id}, this.checked)" style="width:16px; height:16px;">
                Controlar
              </label>
              <div style="display:flex; align-items:center; gap:4px; opacity:${activo ? '1' : '0.4'}; pointer-events:${activo ? 'auto' : 'none'};">
                <button class="btn btn-sm btn-secondary" onclick="ajustarStockBarra(${p.id}, -1)" style="padding:4px 10px; font-size:1rem; font-weight:bold;">-</button>
                <input type="number" min="0" value="${stock}" onchange="fijarStockBarra(${p.id}, this.value)" style="width:50px; text-align:center; padding:4px; border:1px solid #CBD5E1; border-radius:6px; font-weight:bold;">
                <button class="btn btn-sm btn-secondary" onclick="ajustarStockBarra(${p.id}, 1)" style="padding:4px 10px; font-size:1rem; font-weight:bold;">+</button>
                <button class="btn btn-sm btn-danger" onclick="fijarStockBarra(${p.id}, 0)" style="padding:4px 8px; font-size:0.75rem; font-weight:bold;">🚫 0</button>
              </div>
            </div>
          </div>
        `;
    ...
    async function toggleStockBarra(prodId, activo) {
      const p = productosBarra.find(x => x.id === prodId);
    ...
    async function ajustarStockBarra(prodId, delta) {
      const p = productosBarra.find(x => x.id === prodId);
      if (!p) return;
    ...
    async function fijarStockBarra(prodId, valor) {
      const p = productosBarra.find(x => x.id === prodId);
      if (!p) return;
    ...
    socket.on('stock:actualizado', (data) => {
      const p = productosBarra.find(x => x.id === data.producto_id);
      if (p) {

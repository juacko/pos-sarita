            ...
            ${(!pedido.items || pedido.items.length === 0) ? '<p style="color:var(--gray);">Sin items</p>' : (() => {
              ...
              const itemsHtml = activos.map(item => {
                ...
                <div class="comanda-item ${item.estado === 'LISTO' ? 'listo' : ''}">
                  <input type="checkbox" class="comanda-item-check" onchange="marcarItemCocinado(${item.id}, this.checked)" ${item.estado === 'LISTO' ? 'checked' : ''}>
    ...
    async function marcarListo(pedidoId, pendientes) {
      if (pendientes > 0) {
        const ok = await customConfirm({
          ...
          message: `Hay ${pendientes} item(s) sin marcar. ¿Marcar todo como listo de todas formas?`,
